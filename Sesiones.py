...
def buscarcedula(buscar):
    ...
    try:
        conn = connect()
        cur = conn.cursor()
        query = "SELECT paciente.cedula,paciente.nombre,carrera.nombre,paciente.sexo,paciente.edad,paciente.ocupacion,paciente.estado_civil FROM paciente paciente join carrera on carrera=id WHERE cedula='{0}'".format(
            buscar)
...
def obtener_historial(buscar):
    ...
    try:
        ...
        query = "SELECT to_char(fecha, 'YYYY-MM-DD'),asistencia,motivo_ausencia,responsable from cita where paciente='{0}'".format(
            buscar)
