...
title_dict = {'encounter_type':"Tipo de atención", 'faculty_dependence':"Facultad/Dependencia",'patient_type':"Tipo de paciente"}
...
def show_appointment_metrics_fullfilled(month):
    ...
    appointment_list = get_appointment_report(st.session_state.db_engine, get_month_number(month), 2023)
    if appointment_list:
        df_appointment_list = pd.DataFrame([(a.appointment_type, a.status, a.reason) for a in appointment_list],
...
def show_table(month):
    ...
    builder.configure_default_column(groupable=True, sorteable=True, enableRowGroup=True, enablePivot=True, enableValue=True,
