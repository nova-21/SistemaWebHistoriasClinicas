...
if st.session_state.user_info:
        ...
        with base:
            with st.form(key="appointment_register", clear_on_submit=True):
                if st.session_state.id_for_appointment != "":
                    patient_id = st.text_input(
                        "Cédula o identificación del paciente",
                        value=st.session_state.id_for_appointment,
                    )
                else:
            ...
            if submit:
                try:
                    ...
                    st.error(
                        "La identificación se encuentra vacía, revise los datos e intente nuevamente."
                    )
                    time.sleep(2)
    ...
    if (
        ...
        or message == "El paciente no se encuentra registrado"
    ):
        base.empty()
        st.error(message)
        time.sleep(2)
